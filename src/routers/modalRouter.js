...
modalRouter.post("/", async (req, res) => {
  ...
  const mailData = {
    ...
    to: usersMail,
    subject: "Запрос прайса Premium Beef",
    text: " ",

    html: `<b> Посетитель сайта https://premiumbeef.ru/ указал имя: ${name} и запросил актуальные цены. </b><br> Информацию ему необходимо выслать на почту ${email}. Для уточнения данных пользователь оставил номер телефона: ${phone} <br/> `,
  };
